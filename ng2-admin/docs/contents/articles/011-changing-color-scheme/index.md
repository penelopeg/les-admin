...
For light themes we suggest taking `src/app/theme/sass/conf/colorSchemes/_mint.scss` and for 
dark `src/app/theme/sass/conf/colorSchemes/_blue.scss`.
As we want a dark theme, we're taking `blue`.

1) Copy `src/app/theme/sass/conf/colorSchemes/_mint.scss` to `src/app/theme/sass/conf/colorSchemes/_dark.scss`:
<br><br>
